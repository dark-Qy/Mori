...
| Current revision | G8 authority and convergence foundation checkpoint |
| Active goal | G8 — Compose the shared product loop into both apps and add honest cross-process Simulator evidence; G4 motion merge remains deferred |
| Goal state | G5–G7 platform experiences are implemented; the G8 ledger, settlement, deletion, authority, projection, and deterministic harness foundation is reviewed and verified |
| Contract | G0–G3 and G5–G7 committed; G8 foundation ready in this checkpoint; black G4 handoff ready but not merged; white Mori deferred |
| Simulator validation | Watch Debug/Release, 23/23 Watch UI tests on 46 mm, 40 mm Watch core journeys, iPhone product/a11y journeys, Release boundary, and iPhone Computer Use passes |
...
| G6 iPhone experience | Mock-first checkpoint committed | `571bd85` | Four-tab native hierarchy; full-scene Mori presence; one recommended Today task with exact facts; sealed-memory-only history; coin catalog and idempotent purchase/equip; native Settings with companion/reminder/quiet hours, sharing, local data mode, honest automatic-sync status, Apple Settings recovery, and fenced all-data deletion; real/Mock profile isolation; authority-atomic sensing settlement; durable real identity; runtime 7/7, phone unit 17/17, iPhone 17 UI 9/9, and iPhone 17e core/a11y coverage; Debug/Release, strict checks, Release boundary, Computer Use, and independent review PASS | Real conversation, remote/social deletion processors, Watch deletion/sync transport, G4 motion, adaptive appearance matrix, and physical-device behavior remain owned by G7–G9 or `DEVICE_UNVERIFIED` |
| G7 Mori conversation | Mock-first checkpoint committed | `dc4d6a7` | Local-only Chat V1 runtime; streaming, stop, retry, deterministic failures, privacy scanning and warning confirmation; presentation-only drafts; profile-local Mock memory choice; bounded canonical repository; disk revision checks; stale-read rejection; artifact and cross-profile global retirement fences; Mock reset cleanup; native iPhone conversation/settings UX; independent storage and UI reviews | Production provider/gateway, raw-wire decoder, authoritative ledger context builder, final authority-bound remote commit, cleanup retry journal, Watch conversation, physical network/background/device behavior, and G4 motion remain deferred |
| G8 End-to-end product loops | Foundation implemented; app composition pending | This checkpoint | CAS product ledger and global authority; typed deletion roots and exact legacy Mock migration; fail-closed v1/v2 consent compatibility; deterministic task settlement and repair; ticketed global deletion; content-free projection/digest; independent bidirectional three-seed harness; 172 CompanionCore + 285 AppRuntime tests; final review PASS | Replace both legacy app Mock repositories with the shared runtime, add Debug-only cross-process experience/preference/consent relay, paired UI journeys, 10-run targeted flake audit, and Computer Use |
...
| `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --package-path Packages/AppRuntime` | G7 Mock-first conversation tree | PASS: 249 tests in 35 suites, including privacy scanning, consent leases, bounded persistence, disk CAS, stale reads/writers, malformed global fences, cross-profile deletion, and Mock namespace removal |
| `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --package-path Packages/CompanionCore` | G7 Mock-first conversation tree | PASS: 165 tests in 34 suites, including strict recent-message and memory-excerpt boundaries |
| `PhoneAppTests` | G7 Mock-first conversation tree | PASS: 20/20, including legacy private-field scrub, profile-local Mock memory context, and no global consent expansion |
| G7 conversation UI journeys | G7 Mock-first conversation tree | PASS: 15/15 on iPhone 17e, including credential block across relaunch, contact warning, normal persistence without task mutation, offline retry without duplicate turn, malformed/oversized fail-closed behavior, slow-stream stop without partial persistence, confirmed conversation clear, destructive all-data deletion, and accessibility audits |
| `Scripts/check`, `Scripts/test-release-boundaries`, and `git diff --check` | G7 Mock-first conversation tree | PASS; strict source checks and credential scan pass, and Release ships no chat fault selector or Mock fixture surface |
| Computer Use visual and functional review | G7 Mock-first conversation tree | PASS on iPhone 17e for full-scene Mori home, local conversation reply, native settings hierarchy, Mock/real isolation wording, local-only memory context, and conversation-clear confirmation; destructive confirmation was not executed |
| Independent G7 storage review | G7 Mock-first conversation tree | PASS; P0=0, P1=0 after disk revision checks, stale-cache refresh, artifact retirement, cross-profile global retirement, malformed-manifest fail-closed behavior, and fence-first deletion |
| Independent G7 iPhone review | G7 Mock-first conversation tree | PASS; P0=0, P1=0 after closing credential-draft persistence, send and profile-switch races, Mock consent isolation, clear confirmation/result scope, conditional streaming follow, and accessibility semantics |
| `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --package-path Packages/CompanionCore` | G8 foundation tree | PASS: 172 tests in 34 suites, including ProfileLedger CAS, concurrent repositories, legacy Mock migration, and migration/append races |
| `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --package-path Packages/AppRuntime` | G8 foundation tree | PASS: 285 tests in 38 suites, including settlement repair, ticketed deletion, consent version compatibility, and the independent bidirectional three-seed product-loop harness |
| iPhone and Watch generic Simulator builds | G8 foundation tree | PASS: Debug and Release for both schemes |
| `Scripts/check`, `Scripts/test-release-boundaries`, and `git diff --check` | G8 foundation tree | PASS; strict source and credential checks pass, and Release contains no fixture resources, selectors, or Mock identifiers |
| Independent G8 foundation review | final corrected foundation tree | PASS; P0=0, P1=0 after four corrective rounds closed stale consent resurrection, deletion obligation loss, CAS races, oracle self-proof, exact legacy migration, per-ticket ACK, mixed-version fail-closed consent, and ledger migration lost updates |
...
| Initial G7 cross-module review | first complete conversation tree | FAIL: stale repositories could restore cleared/deleted private content; consent, memory, profile-switch, Mock reset, draft, send, clear, streaming, and accessibility races remained | Every Mock-first counterexample was reproduced or traced, fixed, and converted to storage, unit, UI, or Computer Use evidence |
| G7 conversation storage re-review | final conversation storage tree | PASS; P0=0, P1=0 | 249 AppRuntime tests; disk CAS, cache revision validation, content-free artifact/global retirement fences, unconditional fence-first deletion, and malformed-manifest fail-closed behavior |
| G7 iPhone corrective review | final Mock-first conversation UI tree | PASS; P0=0, P1=0 | 20 phone unit tests, Debug/Release builds, blocked-draft relaunch, clear confirmation, slow-stream cancellation, conditional bottom following, profile-local memory authority, and final Computer Use evidence |
| G8 foundation corrective reviews | iterative authority and convergence tree | Initial passes found five, then three, then two P1 groups across deletion-root authority, consent, deletion obligations, ledger CAS, task settlement, and independent oracles | Each counterexample was reproduced, fixed, and added to deterministic regression coverage |
| G8 foundation final review | final foundation tree | PASS; P0=0, P1=0 | 457 package tests, strict checks, Release boundary, and four Debug/Release app builds passed; app composition and cross-process Simulator transport remain explicitly pending |
