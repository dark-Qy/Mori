Create Codex pet row `running-left` for `bili-22`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 只生成 22 娘：深钴蓝超长发、闪电形呆毛、品红眼睛、刘海旁白色矩形表情发夹、白色短袖上衣、深蓝领带与裙装、白手套和白色未来感鞋。绝不能出现浅蓝发的 33 娘或双人组合。. Preserve silhouette, face, palette, material, proportions, markings, and props.
