Create one horizontal runtime animation strip for character `bili_33`, action `touch_body`.
...
Identity lock: 只生成 33 娘：冰浅蓝短发与高侧马尾、弯曲呆毛、品红眼睛、刘海旁白色矩形表情发夹、黑色蝴蝶结状连接件与白色圆形播放图案发饰、白蓝无袖上衣、深蓝裙装、白手套和白色未来感鞋。绝不能出现深蓝长发的 22 娘或双人组合。
...
Motion: Frame 00 starts in the approved neutral stance. Frame 01 shows a tiny surprised blink. Frames 02-04 turn the head and upper torso slightly toward the touch, then lean closer with a friendly smile. Frames 05-07 relax and return smoothly to the exact neutral stance.
...
Background: exactly one uniform #FF00FF color with no gradient, texture, reflection, floor plane, lighting variation, or shadow. Never use #FF00FF inside the character.
