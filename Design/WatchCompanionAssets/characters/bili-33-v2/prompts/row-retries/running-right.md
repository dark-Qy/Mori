Create Codex pet row `running-right` for `bili-33`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 只生成 33 娘：冰浅蓝短发与高侧马尾、弯曲呆毛、品红眼睛、刘海旁白色矩形表情发夹、黑色蝴蝶结状连接件与白色圆形播放图案发饰、白蓝无袖上衣、深蓝裙装、白手套和白色未来感鞋。绝不能出现深蓝长发的 22 娘或双人组合。. Preserve silhouette, face, palette, material, proportions, markings, and props.
